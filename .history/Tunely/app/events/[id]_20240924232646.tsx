...
export default function EventDetails() {
  ...
  const openMaps = async () => {
    ...
    let googleMapsURL = `comgooglemaps://?q=${label}&center=${latitude},${longitude}`;
    let appleMapsURL = `http://maps.apple.com/?ll=${latitude},${longitude}&q=${label}`;
    let defaultMapsURL = '';

    if (Platform.OS === 'ios') {
      defaultMapsURL = appleMapsURL;
    } else {
      defaultMapsURL = `geo:${latitude},${longitude}?q=${latitude},${longitude}(${label})`;
    }

    try {
      // First, try to open Google Maps
      const supported = await Linking.canOpenURL(googleMapsURL);
      if (supported) {
        await Linking.openURL(googleMapsURL);
      } else {
        // If Google Maps is not installed, open Apple Maps or default maps
        const fallbackSupported = await Linking.canOpenURL(defaultMapsURL);
        if (fallbackSupported) {
          await Linking.openURL(defaultMapsURL);
        } else {
          // If neither is supported, open browser-based Google Maps
...
const styles = StyleSheet.create({
  ...
  backButton: {
    position: 'absolute',
    top: Platform.OS === 'ios' ? 50 : 30, // Adjust based on your header height and status bar
    left: 20,
